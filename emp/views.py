...
class ClockInAPIView(APIView):
    ...
    def post(self, request):
        ...
        s = serializers.AttendanceReadSerializer(at)
...
class ClockOutAPIView(APIView):
    ...
    def post(self, request):
        attendance, error = AttendanceService.clock_out(request.user)
        ...
        return Response(
            serializers.AttendanceReadSerializer(attendance).data,
            status=200
        )
...
class TodayAttendanceView(APIView):
    ...
    def get(self, request):
        qs = AttendanceQueryService.get_today_attendance(request.user)
        if qs is None:
...
class HRTLActionAPIView(APIView):
    ...
    def post(self, request, leave_id):
        ...
        if user.role in (LoginUser.ROLE_HR, LoginUser.ROLE_MANAGEMENT):
...
class HRDashboardStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        total_work_seconds = (
            monthly_attendance_qs.aggregate(
